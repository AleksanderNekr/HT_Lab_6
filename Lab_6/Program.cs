...
internal static class Program
{
    ...
    private static void MenuArCreate(ref char[][] jaggedArrChars)
    {
        while (true)
        {
            ...
            switch (choice)
            {
                case "1":
                    ...
                    return;
                case "2":
                    Console.WriteLine("\nВыбрано сгенерировать элементы массива с помощью датчика случайных чисел");
    ...
    private static void DeleteRowMore3Vowels(ref char[][] jaggedArray)
    {
        if (jaggedArray.Length > 0)
        {
            ...
            Console.WriteLine("\nНе найдено строки, в которой есть не менее 3 гласных букв!\nУдаление невозможно!");
        }
        else
    ...
    private static void MenuChooseString(out string str)
    {
        string[] strings =
        {
            ...
            "Классы: StringBuilder и String. Они предоставляют достаточную, функциональность: для работы!",

            "Они предоставляют достаточную, функциональность: для работы; со строками! Однако .NET предлагает еще один"
          + " мощный инструмент.",

            "Однако .NET предлагает; еще один: мощный инструмент - регулярные выражения! Регулярные выражения"
          + " представляют, эффективный. Также гибкий метод: по обработке больших, текстов!",

            "Однако .NET предлагает; еще один: мощный инструмент - регулярные выражения! Позволяя в то же время" +
            " существенно, уменьшить: объемы кода! По сравнению с использованием, стандартных строковых операций?"
        };
    ...
    private static string[] FindSentEndWithExclPoint(string inputStr)
    {
        ...
        if (matches.Count > 0)
        {
            ...
            Console.WriteLine("\nСтрока не содержит корректных предложений, оканчивающихся на ‘!’");
